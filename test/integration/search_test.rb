...
class UsersSearchTest < ActionDispatch::IntegrationTest
  ...
  test "micropost feed search" do
    ...
    assert_select "input[type=submit]"
    assert_select "input[type=search]"
    assert_match "my protein", response.body
    assert_match "zavas milk", response.body
    get "/search", params: { search: "my protein" }
    assert_template 'search_posts'
    assert_match "my protein", response.body
    assert_no_match "zavas milk", response.body
  end
